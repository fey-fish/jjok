...
SMODS.Joker {
    ...
    loc_txt = { name = '{C:tarot}Kenjaku',
        text = {
            ...
            '{C:inactive}(Increase by {C:dark_edition}#2#{C:inactive} on defeating the Ante)'
        } },
    loc_vars = function(self, info_queue, center)
        if G.kenny ~= nil then
            center.ability.extra.card_limit = G.kenny.config.card_limit
        end
        return {
            vars = {
    ...
    config = { extra = { area = nil, card_limit = 2, increase = 1 } },
    ...
    calculate = function(self, card, context)
        if context.boss_defeat then
            G.kenny.config.card_limit = G.kenny.config.card_limit + card.ability.extra.increase
        end
    end,
...
SMODS.Consumable {
    ...
    can_use = function(self, card)
        for i, v in ipairs(G.jokers.cards) do
            if v.config.center.key == 'j_jjok_kenny' then
                if #G.kenny.cards < G.kenny.config.card_limit and #G.jokers.highlighted == 1 and G.jokers.highlighted[1].config.center.key ~= 'j_jjok_kenny' then
    ...
    use = function(self, card)
        G.jokers:remove_card(card.ability.extra._card)
        G.kenny:emplace(card.ability.extra._card)
    end
}
...
SMODS.Enhancement {
    ...
    loc_txt = { name = 'Resonated',
        text = { '{C:white,X:chips}X1.2{} Chips when scored' } }
}
    ...
    calculate = function(self, card, context)
        if context.main_scoring and context.cardarea == G.play then
            if G.GAME.blind.chips > get_blind_amount(G.GAME.round_resets.ante) then
                ...
                return {
                    message = { 'X0.99 Blind Size', colour = G.C.CHIPS }
                }
            end
        end
    end
...
SMODS.Consumable {
    ...
    loc_txt = { name = 'Sword of Extermination',
        text = {
            ...
            'to {C:attention}destroy{} all jokers, else {C:attention}duplicate',
            'all jokers {C:inactive}(does not copy editions)' } },
    config = { extra = { odds = 2 } },
    loc_vars = function(self, info_queue, center)
    ...
    use = function(self, card)
        if pseudorandom('possword') < G.GAME.probabilities.normal / card.ability.extra.odds then
            ...
            for i = 1, #_cards do
                SMODS.add_card({ set = 'Joker', area = G.jokers, key = _cards[i].config.center.key })
            end
        end
    end
}
...
SMODS.Consumable {
    ...
    loc_txt = { name = 'Inverted Spear of Heaven',
        text = {
            ...
            'and gain {C:dark_edition}#1# Joker slot',
            '{s:0.8,C:inactive}(Must hold atleast 1 Joker)' } },
    cost = 6,
    loc_vars = function(self, info_queue, center)
        return {
            vars = {
...
SMODS.Consumable {
    ...
    loc_txt = { name = 'Uzumaki',
        text = { '{C:attention}Fuse Jokers{} using Cursed',
            ...
            '{C:attention}4 {C:common}Common {C:attention}Jokers{} = {C:uncommon}Uncommon',
            '{C:attention}3 {C:uncommon}Uncommon {C:attention}Jokers{} = {C:rare}Rare',
            '{C:attention}2 {C:rare}Rare {C:attention}Jokers{} = {C:legendary}Legendary', } },
    set = 'Tarot',
...
SMODS.Consumable {
    ...
    can_use = function(self, card)
        ...
        if card.ability.extra.yuji_pre ~= nil then
            return true
        elseif card.ability.extra.suku_pre ~= nil and G.jokers.cards[card.ability.extra.suku_pre].ability.extra.fingers < 20 then
            return true
        end
    end,
    use = function(self, card)
        if card.ability.extra.yuji_pre ~= nil then
            G.jokers.cards[card.ability.extra.yuji_pre]:start_dissolve()
...
SMODS.Joker {
    ...
    loc_vars = function(self, info_queue, center)
        info_queue[#info_queue + 1] = G.P_CENTERS.j_jjok_puppet
        return {
            vars = {
...
SMODS.Joker {
    key = 'uruame',
    loc_txt = { name = 'Uruame',
        text = { 'Fulfill conditions for rewards',
            '#1#',
            '#2#' } },
    rarity = 3,
    cost = 12,
    config = { extra = { cur = 5, conditions = {
        1, 2, 3, 4, 5
    } } },
    loc_vars = function(self, info_queue, center)
        if center.ability.extra.cur == 1 then
            return {
                vars = {
                    'sell a special grade joker',
                    'to create 5 rare jokers' }
            }
        elseif center.ability.extra.cur == 2 then
            info_queue[#info_queue + 1] = G.P_CENTERS.c_jjok_awaken
            return {
                vars = {
                    'one shot the boss blind',
                    'to create an awaken card' }
            }
        ...
        elseif center.ability.extra.cur == 4 then
            ...
            return {
                vars = {
                    'sell this card',
                    'to create a curse womb' }
            }
        elseif center.ability.extra.cur == 5 then
            return {
                vars = {
                    'skip a blind',
                    'to gain 1/4 of current money', }
            }
        end
    end,
    calculate = function(self, card, context)
        if context.setting_blind then
            card.ability.extra.cur = pseudorandom_element(card.ability.extra.conditions, pseudoseed('uruame'))
        end
        ...
        elseif card.ability.extra.cur == 2 then
            if JJOK.one_shot_blind(context) == true and G.GAME.blind.boss == true then
                SMODS.add_card({ set = 'Spectral', area = G.consumeables, key = 'c_jjok_awaken' })
            end
        ...
        elseif card.ability.extra.cur == 4 then
            if context.selling_self then
                SMODS.add_card({ set = 'Joker', area = G.jokers, key = 'j_jjok_womb' })
            end
        elseif card.ability.extra.cur == 5 then
            if context.skip_blind then
                ease_dollars(math.floor((G.GAME.dollars / 4) + 0.5))
            end
        end
    end
}
...
SMODS.Joker {
    ...
    calculate = function(self, card, context)
        if context.destroying_card and context.cardarea == G.play and context.destroying_card == context.full_hand[1] and not context.blueprint then
            ...
            return {
                remove = true,
                message = { '+' .. card.ability.extra.Xmult_gain }
            }
        end
        if context.joker_main then
            return {
...
SMODS.Joker {
    ...
    loc_txt = { name = 'Takuma Ino',
        text = { '{C:attention}Cycles through 3 effects on selecting blind,',
